...
public class Tools1 {
    ...
    private int minus = 0;
    /**
     * 默认日志格式：yyyy-MM-dd HH:mm:ss
     */
    public static final String DATE_FORMART_DEFAULT = "yyyy-MM-dd HH:mm:ss";
     ...
     * @return
     */
    public static java.util.Date ParseDateFromStr(String dateStr, String dateFormart){
    	
    	if(isEmpty(dateStr)){
    		return null;
    	}
    	
    	if(Tools1.isEmpty(dateFormart)){
    		dateFormart = DATE_FORMART_DEFAULT;
    	}
...
//    public static String getSaveStr(String s,int LimitStrlen){boolean m;if(DBConnect.Ver==0)m=false;else m=true;return getLimitLenStr(s,LimitStrlen,m,true);}
    ...
    public static String nowTimeAddMonth(){
     ...
     * @return
     */
    public static String getDateStringDayOfStart(String dateStr) {
    	DateFormat needFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
     ...
     * @return
     */
    public static String getDateStringDayOfEnd(String dateStr) {
    	DateFormat needFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        try {
            DateFormat format = new SimpleDateFormat("yyyy-MM-dd");
            java.util.Date date = format.parse(dateStr);
...
//	}
     ...
     * @return
     */
    public static List<ZgStatistic> reMakeReportData(List<ZgStatistic> statics){
    	
    	if(CollectionUtils.isEmpty(statics)){
    		return new ArrayList<ZgStatistic>();
    	}
    	
    	int number = statics.size();
    	if(number >5)
    		return statics;
    	
    	int begin = (6-number)/2;
    	int end = number + begin;
    	List<ZgStatistic> newstatistics = new ArrayList<ZgStatistic>();
    	boolean flage = true;
    	for(int i = 0;i < 6;i++){
    		if(i > end || i < begin){
    			ZgStatistic zgStatistic = new ZgStatistic();
    			zgStatistic.setBottomGroup(1);
    			zgStatistic.setGroupLeaderId(0);
    			zgStatistic.setGroupName("");
    			zgStatistic.setTotal(null);
    			zgStatistic.setType(0);
    			newstatistics.add(zgStatistic);
    		}else{
    			if(flage){
    				newstatistics.addAll(statics);
    				flage = false;
    			}
    		}
    	}
    ...
    */
	 ...
	 */
	public static String getTimeRange(String time,int num){

		//校验参数
		if(isEmpty(time)|| 0 == num){
			return time;
		}
        ...
        calendar.add(Calendar.DAY_OF_YEAR,num);//日期加10天
        java.util.Date date2=calendar.getTime();
        String reStr = dateFormat.format(date2);
        return reStr;
	}

	/**
	 * 字符串格式的日期计算相差天数:yyyy-MM-dd
	 * @param smdate 较小的天数
	 * @param bdate  较大的天数
	 * @return 相差天数
	 * @author WP
	...
	public static int daysBetween(String smdate, String bdate) throws ParseException{
			...
			//获得较大日期的时间戳
			long bdtime = cal.getTimeInMillis();
			//计算相隔日期
			long between_days = (bdtime - smtime) / (1000 * 3600 * 24);
	 ...
	 * @return
	 */
	public static String formatDateByPattern(java.util.Date date, String dateFormat) {
